...
@Configuration
@EnableWebSocketMessageBroker  //websocket message hendling을 허용
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void configureMessageBroker(MessageBrokerRegistry registry) {
        registry.enableSimpleBroker("/sub"); //메세지 응담 prefix
    ...
    public void registerStompEndpoints(StompEndpointRegistry registry) {
        registry.addEndpoint("/stomp/chat").setAllowedOrigins("http://localhost:830").withSockJS(); // javascript에서 SockJS생성자를 통해 연결
    }
}
